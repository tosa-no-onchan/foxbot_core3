...
float cICM20948_COMBI::invSqrt(float x) {
	float halfx = 0.5f * x;
	float y = x;
	long i = *(long*)&y;
	i = 0x5f3759df - (i>>1);
...
cICM20948_COMBI::cICM20948_COMBI()
{
	calibratingG = 0;
	calibratingA = 0;
  calibratingM = 0;
...
bool cICM20948_COMBI::begin(){
  ...
  mag_process_time  = foxbot3::micros_();
  ...
  SERIAL_PORT.print(F("cICM20948_COMBI::begin(): #99 bConnected="));
  SERIAL_PORT.println(bConnected);
  return bConnected;
}

bool cICM20948_COMBI::begin_icm20948()
{
  int cnt;
  uint8_t data;
...
bool cICM20948_COMBI::begin_bno086()
{
  int cnt;
  uint8_t data;
  ...
  while (!initialized)
  {
    bool rc;
      ...
      rc=myIMU_2.begin(BNO08X_ADDR, Wire, BNO08X_INT, BNO08X_RST);
    #else
      ...
      rc=myIMU_2.beginSPI(BNO08X_CS, BNO08X_INT, BNO08X_RST,3000000,MPU_SPI);
    #endif
  ...
  if(initialized == true){
    bConnected=init_bno086();
  }
  return bConnected;
}

bool cICM20948_COMBI::init(){

  return true;
}

bool cICM20948_COMBI::init_icm20948( void ){
  uint8_t state;
  uint8_t data;
  ...
  #if !defined(USE_DMP_NISHI) && !defined(USE_AGM_NISHI)
    // Gyro and Acc を使う
    #if defined(USE_ACC_NISHI) || defined(USE_GRYO_NISHI) || defined(USE_MAG_X)
...
bool cICM20948_COMBI::init_bno086( void ){
  bool rc=true;

  SERIAL_PORT.println("cICM20948_COMBI::init_bno086(): #1");
    ...
    // 100[hz]  -> 10[ms]
    rc = myIMU_2.enableMagnetometer(10);
    if (rc == false) {
...
bool cICM20948_COMBI::update( void ){
  bool rc0=false;
  ...
  if( bConnected == true ){
  ...
  #if defined(USE_MAG)
    #if defined(USE_DUAL_RATE)
      mag_cur_process_time  = foxbot3::micros_();
    ...
    #endif
    if (myIMU_2.wasReset()) {
      #define USE_UPDATE_DEBUG_2
      #if defined(USE_UPDATE_DEBUG_2)
        SERIAL_PORT.println("cICM20948_COMBI::update(): #2 sensor was reset");
      #endif
      init_bno086();
    }
    // check data comming?
    mag_data_comming = myIMU_2.getSensorEvent();
  #endif
  return rc0|mag_data_comming;
}

void cICM20948_COMBI::gyro_init( void ){
	uint8_t i;
	for( i=0; i<3; i++ ){
    gyroADC[i]  = 0;
		gyroZero[i] = 0;
...
void cICM20948_COMBI::gyro_get_adc( void ){
	int16_t x = 0;
	int16_t y = 0;
	int16_t z = 0;
...
void cICM20948_COMBI::gyro_cali_start(){
	//calibratingG = MPU_CALI_COUNT;
  calibratingG_f = 0;
  calibratingG = 0; 
}

void cICM20948_COMBI::gyro_common(){
	static int32_t g[3];
  ...
  if(calibratingG_f == 0){
    ...
    if(calibratingG >= MPU_CALI_COUNT_GYRO){
      ...
      if(calibratingG >= (MPU_CALI_COUNT_GYRO * 2)-1){
...
void cICM20948_COMBI::acc_init( void ){
  uint8_t i;
  for( i=0; i<3; i++ ){
    accADC[i]   = 0;
		accZero[i]  = 0;
...
void cICM20948_COMBI::acc_get_adc( void ){
	int16_t x = 0;
	int16_t y = 0;
	int16_t z = 0;
  //uint8_t rawADC[6];
...
*/
void cICM20948_COMBI::acc_common(){
	static int32_t a[3];
...
void cICM20948_COMBI::mag_init( void )
{
  uint8_t i;
  for( i=0; i<3; i++ )
  {
...
/*---------------------------------------------------------------------------
     ...
     RET     : void
---------------------------------------------------------------------------*/
bool cICM20948_COMBI::mag_get_adc( void )
{
	float x = 0.0;
	float y = 0.0;
	float z = 0.0;
  ...
  // is it the correct sensor data we want?
  if (mag_data_comming == true && myIMU_2.getSensorEventID() == SENSOR_REPORTID_MAGNETIC_FIELD) {

    x = myIMU_2.getMagX();
    y = myIMU_2.getMagY();
    z = myIMU_2.getMagZ();
    accuracy = myIMU_2.getMagAccuracy();
    ...
    // Dynamic Calibration 機能が働いているので、起動直後から、
    // accuracy=1 以上になるまで、時間がかかるかも知れない!!
    // こちらでのキャリブレーションは、行わない。BNO086 の方に任せる。
    // キャリブレーションが完了したら、accuracy==3 だけにしてみては? 
    if(calibratingM_f != 1){
      if((x==0.0 && y==0.0 && z==0.0)|| accuracy==0){
        //mag_common();
        return false;
      }
    }
    else{
      if((x==0.0 && y==0.0 && z==0.0)|| accuracy!=3){
        //mag_common();
        return false;
      }
    }
    ...
    //#define USE_MAG_DEBUG_X2
      ...
      SERIAL_PORT.print(F("magCalib: "));
      SERIAL_PORT.print(x, 3);
      SERIAL_PORT.print(F(" ,"));
      SERIAL_PORT.print(y, 3);
      SERIAL_PORT.print(F(" ,"));
      SERIAL_PORT.print(z, 3);
...
void cICM20948_COMBI::mag_common()
{
...
void cICM20948_COMBI::dmp_init( void )
{
  uint8_t i;
...
bool cICM20948_COMBI::dmp_cali_get_done()
{
...
bool cICM20948_COMBI::dmp_get_adc(){
...
-----------------*/
bool cICM20948_COMBI::agm_get_adc(){
...
void cICM20948_COMBI::agm_common(){
...
void cICM20948_COMBI::acc_cali_start()
{
...
bool cICM20948_COMBI::acc_cali_get_done()
{
	if( calibratingA_f !=0 ) return true;
	else                    return false;
}

bool cICM20948_COMBI::gyro_cali_get_done()
{
	if( calibratingG_f != 0 ) return true;
	else                    return false;
}

bool cICM20948_COMBI::mag_cali_get_done()
{
