...
#ifndef _ICM20948_COMBI_H_
#define _ICM20948_COMBI_H_
...
// add by nishi 2025.3.24
	...
	//#define MPU_CALI_COUNT_GYRO 300
	#define MPU_CALI_COUNT_GYRO 500
	#define MPU_CALI_COUNT_ACC 500
	#define MPU_CALI_COUNT_MAG 150
	#define MPU_CALI_COUNT_DMP 500
	#define MPU_CALI_COUNT_DMP9 500

	// IMU.cpp で使用する。
	#define IMU_CALI_COUNT_DMP_PRE 500
	#define IMU_CALI_COUNT_DMP 500
#else
	//#define MPU_CALI_COUNT_GYRO 300
	#define MPU_CALI_COUNT_GYRO 150
	#define MPU_CALI_COUNT_ACC 150
	#define MPU_CALI_COUNT_MAG 150
	#define MPU_CALI_COUNT_DMP 200
	#define MPU_CALI_COUNT_DMP9 400
...
public:
	...
	uint32_t begin_update_rate = 100;	// begin時の update() calling rate [Hz]
	uint32_t mag_update_rate = 100;		// Magneto の update() calling rate [Hz]
	uint64_t mag_update_rate_us = 1000000/100;	// Magneto の update() calling rate interval us

	uint64_t mag_prev_process_time;
	uint64_t mag_cur_process_time;
	uint64_t mag_process_time;
	...
	float mag_offsets[3]            = { 28.67 , -3.63 , 3.34 };	// MotionCali #4 4.7%
	//float mag_offsets[3]            = { 253.71 , 27.74 , 221.33 };	// calibrate3.py
	...
	//									{ 2.19322 , -0.15711 , 0.23466 },
	//									{ -0.15711 , 2.20862 , 0.36833 },
	//									{ 0.23466 , 0.36833 , 2.93345 }};
...
public:
	cICM20948_COMBI();
  	...
  	bool begin( void );
  	bool begin_icm20948( void );
  	bool begin_bno086( void );
  	bool init( void );

  	bool init_icm20948( void );
  	bool init_bno086( void );
...
private:
		...
		BNO08x myIMU_2; // If using SPI create an BNO08x SPI object
	#endif
